...
if [ -z "${QCG_OPENROUTER_API_KEY:-}" ]; then
  if [ ! -r "$auth_file" ]; then
    echo "set QCG_OPENROUTER_API_KEY or provide a readable OpenCode auth file at $auth_file" >&2
    exit 2
  fi
  if ! QCG_OPENROUTER_API_KEY="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["openrouter"]["key"])' "$auth_file")"; then
    echo "failed to read the OpenRouter key from $auth_file" >&2
    exit 2
  fi
  export QCG_OPENROUTER_API_KEY
fi
...
# is a derived build artifact, and SKILL.md is generated by the authoring flow,
# so neither belongs to the behavioral reproduction blueprint.
write_blueprint() {
...
for path in sorted(root.rglob("*")):
    ...
    if relative.as_posix() in {"qcg.toml", "SKILL.md"}:
        continue
